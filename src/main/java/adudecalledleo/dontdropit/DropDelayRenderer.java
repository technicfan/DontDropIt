...
public class DropDelayRenderer {
    ...
    public static void renderFavoriteIcon(GuiGraphics context, ItemStack stack, int x, int y) {
        if (!ModConfig.get().favorites.drawOverlay || !FavoredChecker.isStackFavored(stack))
            return;
        context.blit(RenderPipelines.GUI_TEXTURED, TEX_FAVORITE, x, y, 18, 18, 18, 18, 18, 18);
    }
    ...
    public static void renderProgressOverlay(GuiGraphics context, ItemStack stack, int x, int y, int w, int h) {
    ...
    public static void renderOverlay(GuiGraphics context, ItemStack stack, int x, int y) {
        if (stack.isEmpty())
            return;
